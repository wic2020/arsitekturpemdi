...
$ralFields = [
    'kode_ral_1', 'nama_ral_1',
    'kode_ral_2', 'nama_ral_2',
    'kode_ral_3', 'nama_ral_3',
    'kode_ral_4', 'nama_ral_4',
];
$formErrors = [];
$openFormModal = false;
$formMode = 'create';
$formState = array_fill_keys($ralFields, '');
$formState['id'] = '';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (in_array($action, ['create', 'update'], true)) {
        ...
        if ($action === 'update' && !$recordId) {
            $formErrors[] = 'ID data RAL tidak valid.';
        }

        if (!$formErrors) {
            $duplicateSql = 'SELECT id FROM ral WHERE kode_ral_4 = :kode_ral_4';
            $duplicateParams = ['kode_ral_4' => $formState['kode_ral_4']];
            if ($action === 'update') {
            ...
            if ($duplicateStmt->fetch()) {
                $formErrors[] = 'Kode RAL level 4 sudah digunakan.';
            }
        }
        ...
        if (!$formErrors && $action === 'update') {
            $oldStmt = db()->prepare(
                'SELECT id, kode_ral_1, nama_ral_1, kode_ral_2, nama_ral_2,
                        kode_ral_3, nama_ral_3, kode_ral_4, nama_ral_4
                 FROM ral WHERE id = :id LIMIT 1'
            );
            $oldStmt->execute(['id' => $recordId]);
            $oldValues = $oldStmt->fetch();

            if (!$oldValues) {
                $formErrors[] = 'Data RAL yang akan diubah tidak ditemukan.';
            }
        }
        ...
        if (!$formErrors) {
            ...
            try {
                ...
                if ($action === 'create') {
                    $stmt = db()->prepare(
                        'INSERT INTO ral
                            (kode_ral_1, nama_ral_1, kode_ral_2, nama_ral_2,
                             kode_ral_3, nama_ral_3, kode_ral_4, nama_ral_4)
                         VALUES
                            (:kode_ral_1, :nama_ral_1, :kode_ral_2, :nama_ral_2,
                             :kode_ral_3, :nama_ral_3, :kode_ral_4, :nama_ral_4)'
                    ...
                    $description = 'Menambahkan referensi layanan ' . $formState['kode_ral_4'];
                    audit_log(
                        (int) $user['id'],
                        'create',
                        'ral',
                    ...
                    $successMessage = 'Data RAL berhasil ditambahkan.';
                } else {
                    $params['id'] = $recordId;
                    $stmt = db()->prepare(
                        'UPDATE ral SET
                            kode_ral_1 = :kode_ral_1, nama_ral_1 = :nama_ral_1,
                            kode_ral_2 = :kode_ral_2, nama_ral_2 = :nama_ral_2,
                            kode_ral_3 = :kode_ral_3, nama_ral_3 = :nama_ral_3,
                            kode_ral_4 = :kode_ral_4, nama_ral_4 = :nama_ral_4
                         WHERE id = :id'
                    );
                    $stmt->execute($params);
                    $newValues = $params;
                    ...
                    audit_log(
                        ...
                        'ral',
                        (int) $recordId,
                        'Mengubah referensi layanan ' . $formState['kode_ral_4'],
                        $oldValues,
                        $newValues
                    );
                    $successMessage = 'Data RAL berhasil diperbarui.';
                }

                db()->commit();
                set_flash('success', $successMessage);
                redirect('index.php?page=ral');
            } catch (Throwable $exception) {
                db()->rollBack();
                error_log('Penyimpanan RAL gagal: ' . $exception->getMessage());
                $formErrors[] = 'Data RAL gagal disimpan. Silakan coba kembali.';
            }
        }
    } elseif ($action === 'delete') {
        $recordId = filter_var($_POST['id'] ?? null, FILTER_VALIDATE_INT, ['options' => ['min_range' => 1]]);
        if (!$recordId) {
            set_flash('error', 'ID data RAL tidak valid.');
            redirect('index.php?page=ral');
        }
        ...
        if (!$record) {
            set_flash('error', 'Data RAL tidak ditemukan.');
            redirect('index.php?page=ral');
        }

        $referenceStmt = db()->prepare(
            'SELECT
                (SELECT COUNT(*) FROM program WHERE id_ral = :program_id) +
                (SELECT COUNT(*) FROM dal WHERE id_ral = :dal_id) AS total'
        );
        ...
        if ($referenceCount > 0) {
            set_flash('error', "Data {$record['kode_ral_4']} tidak dapat dihapus karena masih digunakan oleh {$referenceCount} data terkait.");
            redirect('index.php?page=ral');
        }

        db()->beginTransaction();
        try {
            db()->prepare('DELETE FROM ral WHERE id = :id')->execute(['id' => $recordId]);
            audit_log(
                (int) $user['id'],
                'delete',
                'ral',
                (int) $recordId,
                'Menghapus referensi layanan ' . $record['kode_ral_4'],
                $record,
                null
            );
            db()->commit();
            set_flash('success', 'Data RAL berhasil dihapus.');
        } catch (Throwable $exception) {
            db()->rollBack();
            error_log('Penghapusan RAL gagal: ' . $exception->getMessage());
            set_flash('error', 'Data RAL gagal dihapus.');
        }
        redirect('index.php?page=ral');
    } else {
        set_flash('error', 'Aksi RAL tidak valid.');
        redirect('index.php?page=ral');
    }
}
...
if ($search !== '') {
    $whereSql = " WHERE CONCAT_WS(' ',
        r.kode_ral_1, r.nama_ral_1, r.kode_ral_2, r.nama_ral_2,
        r.kode_ral_3, r.nama_ral_3, r.kode_ral_4, r.nama_ral_4
    ) LIKE :search";
    $queryParams['search'] = '%' . $search . '%';
}

$countStmt = db()->prepare('SELECT COUNT(*) FROM ral r' . $whereSql);
...
$listStmt = db()->prepare(
    'SELECT r.*,
        (SELECT COUNT(*) FROM program p WHERE p.id_ral = r.id) AS program_count,
        (SELECT COUNT(*) FROM dal d WHERE d.id_ral = r.id) AS business_count
     FROM ral r' . $whereSql . '
     ORDER BY r.kode_ral_4 ASC, r.id ASC
...
$stats = db()->query(
    'SELECT COUNT(*) total,
        COUNT(DISTINCT kode_ral_1) level_1,
        COUNT(DISTINCT kode_ral_2) level_2,
        COUNT(DISTINCT kode_ral_3) level_3
     FROM ral'
)->fetch();

function ral_page_url(int $pageNumber, string $search, int $perPage): string
{
...
<div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
    <div>
        ...
        <h1 class="mt-1 text-2xl font-bold tracking-tight text-slate-900">Referensi Arsitektur Layanan (RAL)</h1>
        <p class="mt-1 text-sm text-slate-500">Kelola struktur referensi layanan pemerintahan hingga level 4.</p>
    </div>
    <div class="flex gap-2">
        <a href="cetak/cetak_referensi.php?jenis=ral" target="_blank" class="inline-flex items-center justify-center gap-2 rounded-md border border-slate-300 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 shadow-sm hover:bg-slate-50">
            <i data-lucide="printer" class="h-4 w-4"></i>Cetak
        </a>
        <button type="button" class="inline-flex items-center justify-center gap-2 rounded-md bg-blue-700 px-4 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-blue-800" data-ral-add>
            <i data-lucide="plus" class="h-4 w-4"></i>Tambah RAL
        </button>
    </div>
</div>
...
<div class="mb-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
    <?php
    $summaryCards = [
        ['label' => 'Total RAL', 'value' => $stats['total'], 'icon' => 'layers-3'],
...
<section class="rounded-lg border border-slate-200 bg-white shadow-sm">
    <div class="flex flex-col gap-3 border-b border-slate-200 px-4 py-3 lg:flex-row lg:items-center lg:justify-between">
        ...
        <form method="get" class="flex flex-col gap-2 sm:flex-row">
            ...
            <?php if ($search !== ''): ?>
                <a href="index.php?page=ral" class="inline-flex items-center justify-center rounded-md px-3 py-2 text-xs font-semibold text-red-700 hover:bg-red-50">Reset</a>
            <?php endif; ?>
        </form>
    </div>
        ...
        <table class="w-full min-w-[1050px] text-left text-xs xl:min-w-0 xl:table-fixed">
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <?php foreach ($rows as $index => $row): ?>
                    ...
                    $recordPayload = [
                        'id' => (int) $row['id'],
                        'kode_ral_1' => $row['kode_ral_1'],
                        'nama_ral_1' => $row['nama_ral_1'],
                        'kode_ral_2' => $row['kode_ral_2'],
                        'nama_ral_2' => $row['nama_ral_2'],
                        'kode_ral_3' => $row['kode_ral_3'],
                        'nama_ral_3' => $row['nama_ral_3'],
                        'kode_ral_4' => $row['kode_ral_4'],
                        'nama_ral_4' => $row['nama_ral_4'],
                    ...
                    <tr class="align-top hover:bg-slate-50">
                        ...
                        <?php for ($level = 1; $level <= 4; $level++): ?>
                            <td class="break-words px-2.5 py-3">
                                <span class="inline-flex max-w-full break-all rounded bg-slate-100 px-1.5 py-0.5 font-mono text-[10px] font-semibold leading-4 text-slate-700"><?= e($row["kode_ral_{$level}"]) ?></span>
                                <p class="mt-1 text-[11px] leading-4 text-slate-700"><?= e($row["nama_ral_{$level}"]) ?></p>
                            </td>
                        <?php endfor; ?>
                        <td class="px-2 py-3">
                            <div class="flex justify-center gap-1">
                                <button type="button" class="inline-flex h-7 w-7 items-center justify-center rounded border border-slate-300 text-slate-600 hover:bg-slate-100" title="Lihat RAL" aria-label="Lihat <?= e($row['kode_ral_4']) ?>" data-ral-view data-record="<?= e($recordJson) ?>">
                                    <i data-lucide="eye" class="h-3.5 w-3.5"></i>
                                </button>
                                <button type="button" class="inline-flex h-7 w-7 items-center justify-center rounded border border-blue-200 text-blue-700 hover:bg-blue-50" title="Edit RAL" aria-label="Edit <?= e($row['kode_ral_4']) ?>" data-ral-edit data-record="<?= e($recordJson) ?>">
                                    <i data-lucide="pencil" class="h-3.5 w-3.5"></i>
                                </button>
                                <button type="button" class="inline-flex h-7 w-7 items-center justify-center rounded border border-red-200 text-red-700 hover:bg-red-50" title="Hapus RAL" aria-label="Hapus <?= e($row['kode_ral_4']) ?>" data-ral-delete data-id="<?= (int) $row['id'] ?>" data-code="<?= e($row['kode_ral_4']) ?>" data-name="<?= e($row['nama_ral_4']) ?>" data-references="<?= (int) $row['program_count'] + (int) $row['business_count'] ?>">
    ...
    <?php if ($totalRows > 0): ?>
        <div class="flex flex-col gap-3 border-t border-slate-200 px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-5">
            ...
            <?php render_numbered_pagination(
                $currentPage,
                $totalPages,
                static fn(int $pageNumber): string => ral_page_url($pageNumber, $search, $perPage)
            ); ?>
        </div>
    <?php endif; ?>
</section>

<div class="fixed inset-0 z-50 hidden items-center justify-center bg-slate-950/70 px-3 py-4 backdrop-blur-sm sm:px-6" data-modal="ral-form" role="dialog" aria-modal="true" aria-labelledby="ral-form-title">
    <form method="post" class="flex max-h-[94vh] w-full max-w-4xl flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl">
        <?= csrf_field() ?>
        <input type="hidden" name="action" value="<?= e($formMode) ?>" data-ral-action>
        <input type="hidden" name="id" value="<?= e($formState['id']) ?>" data-ral-id>
        ...
        <div class="flex shrink-0 items-start justify-between gap-4 border-b border-slate-200 bg-gradient-to-r from-slate-50 to-white px-5 py-4 sm:px-6 sm:py-5">
            <div class="flex min-w-0 items-start gap-3.5">
                ...
                <div class="min-w-0">
                    <h2 id="ral-form-title" class="text-lg font-bold tracking-tight text-slate-900" data-ral-form-title>Tambah Data RAL</h2>
            ...
            <div class="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
                ...
                <?php for ($level = 1; $level <= 4; $level++): ?>
                    <fieldset class="grid gap-3 border-b border-slate-100 px-4 py-3.5 last:border-b-0 sm:grid-cols-[100px_210px_minmax(0,1fr)] sm:items-center sm:gap-4">
                        ...
                        <div>
                            <label for="kode_ral_<?= $level ?>" class="mb-1.5 block text-[10px] font-semibold uppercase tracking-wide text-slate-500 sm:sr-only">Kode level <?= $level ?></label>
                            <div class="relative">
                                <i data-lucide="hash" class="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400"></i>
                                <input id="kode_ral_<?= $level ?>" name="kode_ral_<?= $level ?>" type="text" maxlength="50" value="<?= e($formState["kode_ral_{$level}"]) ?>" placeholder="RAL.00" class="w-full rounded-lg border border-slate-300 bg-white py-2.5 pl-9 pr-3 font-mono text-sm uppercase text-slate-900 shadow-sm outline-none transition placeholder:text-slate-300 focus:border-blue-600 focus:ring-4 focus:ring-blue-100/70" data-ral-field="kode_ral_<?= $level ?>" required>
                            </div>
                        </div>

                        <div>
                            <label for="nama_ral_<?= $level ?>" class="mb-1.5 block text-[10px] font-semibold uppercase tracking-wide text-slate-500 sm:sr-only">Nama referensi level <?= $level ?></label>
                            <input id="nama_ral_<?= $level ?>" name="nama_ral_<?= $level ?>" type="text" maxlength="255" value="<?= e($formState["nama_ral_{$level}"]) ?>" placeholder="Masukkan nama referensi level <?= $level ?>" class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2.5 text-sm text-slate-900 shadow-sm outline-none transition placeholder:text-slate-300 focus:border-blue-600 focus:ring-4 focus:ring-blue-100/70" data-ral-field="nama_ral_<?= $level ?>" required>
                        </div>
                    </fieldset>
                <?php endfor; ?>
            </div>
        ...
        <div class="flex shrink-0 flex-col-reverse gap-2 border-t border-slate-200 bg-white px-5 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-6">
            ...
            <div class="flex justify-end gap-2.5">
                ...
                <button type="submit" class="inline-flex items-center gap-2 rounded-lg bg-blue-700 px-5 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-blue-800 focus:outline-none focus:ring-4 focus:ring-blue-100">
                    ...
                    <span data-ral-submit-label>Simpan Data</span>
                </button>
            </div>
        </div>
    </form>
...
<div class="fixed inset-0 z-50 hidden items-center justify-center bg-slate-950/70 px-3 py-4 backdrop-blur-sm sm:px-6" data-modal="ral-view" role="dialog" aria-modal="true" aria-labelledby="ral-view-title">
    <div class="flex max-h-[94vh] w-full max-w-4xl flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl">
        <div class="flex shrink-0 items-start justify-between gap-4 border-b border-slate-200 bg-gradient-to-r from-slate-50 to-white px-5 py-4 sm:px-6 sm:py-5">
            <div class="flex min-w-0 items-start gap-3.5">
                ...
                <div class="min-w-0">
                    <h2 id="ral-view-title" class="text-lg font-bold tracking-tight text-slate-900">Detail Data RAL</h2>
            ...
            <?php for ($level = 1; $level <= 4; $level++): ?>
                <section class="relative overflow-hidden rounded-xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
                    ...
                    <div class="relative">
                        ...
                        <p class="mt-4 inline-flex rounded-md bg-red-50 px-2 py-1 font-mono text-xs font-bold text-red-700 ring-1 ring-inset ring-red-100" data-view-field="kode_ral_<?= $level ?>"></p>
                        <p class="mt-2 text-sm font-medium leading-6 text-slate-800" data-view-field="nama_ral_<?= $level ?>"></p>
                    </div>
                </section>
            <?php endfor; ?>
        </div>
        <div class="grid shrink-0 gap-3 border-t border-slate-200 bg-white px-5 py-4 text-sm sm:grid-cols-2 sm:px-6">
            ...
            <div class="flex items-center gap-3 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2.5">
                ...
                <p class="text-slate-600">Domain layanan terkait <strong class="ml-1 text-slate-900" data-view-businesses>0</strong></p>
            </div>
        </div>
...
<div class="fixed inset-0 z-50 hidden items-center justify-center bg-slate-950/70 px-3 py-4 backdrop-blur-sm sm:px-6" data-modal="ral-delete" role="dialog" aria-modal="true" aria-labelledby="ral-delete-title">
    <form method="post" class="w-full max-w-md overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl">
        ...
        <div class="p-6 sm:p-7">
            ...
            <h2 id="ral-delete-title" class="mt-5 text-xl font-bold tracking-tight text-slate-900">Hapus Data RAL?</h2>
...
(() => {
    ...
    const formModal = document.querySelector('[data-modal="ral-form"]');
    const viewModal = document.querySelector('[data-modal="ral-view"]');
    const deleteModal = document.querySelector('[data-modal="ral-delete"]');
    const fields = <?= json_encode($ralFields, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES) ?>;
    ...
    const fillForm = (record = {}) => {
        fields.forEach((field) => {
            const input = formModal.querySelector(`[data-ral-field="${field}"]`);
            if (input) input.value = record[field] || '';
        });
        formModal.querySelector('[data-ral-id]').value = record.id || '';
    };

    document.querySelector('[data-ral-add]')?.addEventListener('click', () => {
        fillForm();
        formModal.querySelector('[data-ral-action]').value = 'create';
        formModal.querySelector('[data-ral-form-title]').textContent = 'Tambah Data RAL';
        formModal.querySelector('[data-ral-submit-label]').textContent = 'Simpan Data';
        openModal(formModal);
    });

    document.querySelectorAll('[data-ral-edit]').forEach((button) => {
        button.addEventListener('click', () => {
            const record = JSON.parse(button.dataset.record || '{}');
            fillForm(record);
            formModal.querySelector('[data-ral-action]').value = 'update';
            formModal.querySelector('[data-ral-form-title]').textContent = 'Edit Data RAL';
            formModal.querySelector('[data-ral-submit-label]').textContent = 'Simpan Perubahan';
            openModal(formModal);
        });
    });

    document.querySelectorAll('[data-ral-view]').forEach((button) => {
        button.addEventListener('click', () => {
            const record = JSON.parse(button.dataset.record || '{}');
            viewModal.querySelector('[data-view-code]').textContent = `${record.kode_ral_4 || ''} · ${record.nama_ral_4 || ''}`;
            fields.forEach((field) => {
    ...
    document.querySelectorAll('[data-ral-delete]').forEach((button) => {
        button.addEventListener('click', () => {
    ...
    formModal.querySelector('[data-ral-form-title]').textContent = <?= json_encode($formMode === 'update' ? 'Edit Data RAL' : 'Tambah Data RAL') ?>;
    formModal.querySelector('[data-ral-submit-label]').textContent = <?= json_encode($formMode === 'update' ? 'Simpan Perubahan' : 'Simpan Data') ?>;
    openModal(formModal);
    <?php endif; ?>
})();
</script>
