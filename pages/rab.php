...
$rabFields = [
    'kode_rab_1', 'nama_rab_1',
    'kode_rab_2', 'nama_rab_2',
    'kode_rab_3', 'nama_rab_3',
    'kode_rab_4', 'nama_rab_4',
];
$formErrors = [];
$openFormModal = false;
$formMode = 'create';
$formState = array_fill_keys($rabFields, '');
$formState['id'] = '';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (in_array($action, ['create', 'update'], true)) {
        ...
        if ($action === 'update' && !$recordId) {
            $formErrors[] = 'ID data RAB tidak valid.';
        }

        if (!$formErrors) {
            $duplicateSql = 'SELECT id FROM rab WHERE kode_rab_4 = :kode_rab_4';
            $duplicateParams = ['kode_rab_4' => $formState['kode_rab_4']];
            if ($action === 'update') {
            ...
            if ($duplicateStmt->fetch()) {
                $formErrors[] = 'Kode RAB level 4 sudah digunakan.';
            }
        }
        ...
        if (!$formErrors && $action === 'update') {
            $oldStmt = db()->prepare(
                'SELECT id, kode_rab_1, nama_rab_1, kode_rab_2, nama_rab_2,
                        kode_rab_3, nama_rab_3, kode_rab_4, nama_rab_4, id_program
                 FROM rab WHERE id = :id LIMIT 1'
            );
            $oldStmt->execute(['id' => $recordId]);
            $oldValues = $oldStmt->fetch();

            if (!$oldValues) {
                $formErrors[] = 'Data RAB yang akan diubah tidak ditemukan.';
            }
        }
        ...
        if (!$formErrors) {
            ...
            try {
                ...
                if ($action === 'create') {
                    $stmt = db()->prepare(
                        'INSERT INTO rab
                            (kode_rab_1, nama_rab_1, kode_rab_2, nama_rab_2,
                             kode_rab_3, nama_rab_3, kode_rab_4, nama_rab_4)
                         VALUES
                            (:kode_rab_1, :nama_rab_1, :kode_rab_2, :nama_rab_2,
                             :kode_rab_3, :nama_rab_3, :kode_rab_4, :nama_rab_4)'
                    );
                    ...
                    $description = 'Menambahkan referensi bisnis ' . $formState['kode_rab_4'];
                    audit_log(
                        (int) $user['id'],
                        'create',
                        'rab',
                    ...
                    $successMessage = 'Data RAB berhasil ditambahkan.';
                } else {
                    $params['id'] = $recordId;
                    $stmt = db()->prepare(
                        'UPDATE rab SET
                            kode_rab_1 = :kode_rab_1, nama_rab_1 = :nama_rab_1,
                            kode_rab_2 = :kode_rab_2, nama_rab_2 = :nama_rab_2,
                            kode_rab_3 = :kode_rab_3, nama_rab_3 = :nama_rab_3,
                            kode_rab_4 = :kode_rab_4, nama_rab_4 = :nama_rab_4
                         WHERE id = :id'
                    );
                    $stmt->execute($params);
                    $newValues = $params;
                    ...
                    audit_log(
                        ...
                        'rab',
                        (int) $recordId,
                        'Mengubah referensi bisnis ' . $formState['kode_rab_4'],
                        $oldValues,
                        $newValues
                    );
                    $successMessage = 'Data RAB berhasil diperbarui.';
                }

                db()->commit();
                set_flash('success', $successMessage);
                redirect('index.php?page=rab');
            } catch (Throwable $exception) {
                db()->rollBack();
                error_log('Penyimpanan RAB gagal: ' . $exception->getMessage());
                $formErrors[] = 'Data RAB gagal disimpan. Silakan coba kembali.';
            }
        }
    } elseif ($action === 'delete') {
        $recordId = filter_var($_POST['id'] ?? null, FILTER_VALIDATE_INT, ['options' => ['min_range' => 1]]);
        if (!$recordId) {
            set_flash('error', 'ID data RAB tidak valid.');
            redirect('index.php?page=rab');
        }
        ...
        if (!$record) {
            set_flash('error', 'Data RAB tidak ditemukan.');
            redirect('index.php?page=rab');
        }

        $referenceStmt = db()->prepare(
            'SELECT
                (SELECT COUNT(*) FROM program WHERE id_rab = :program_id) +
                (SELECT COUNT(*) FROM dab WHERE id_rab = :dab_id) AS total'
        );
        ...
        if ($referenceCount > 0) {
            set_flash('error', "Data {$record['kode_rab_4']} tidak dapat dihapus karena masih digunakan oleh {$referenceCount} data terkait.");
            redirect('index.php?page=rab');
        }

        db()->beginTransaction();
        try {
            db()->prepare('DELETE FROM rab WHERE id = :id')->execute(['id' => $recordId]);
            audit_log(
                (int) $user['id'],
                'delete',
                'rab',
                (int) $recordId,
                'Menghapus referensi bisnis ' . $record['kode_rab_4'],
                $record,
                null
            );
            db()->commit();
            set_flash('success', 'Data RAB berhasil dihapus.');
        } catch (Throwable $exception) {
            db()->rollBack();
            error_log('Penghapusan RAB gagal: ' . $exception->getMessage());
            set_flash('error', 'Data RAB gagal dihapus.');
        }
        redirect('index.php?page=rab');
    } else {
        set_flash('error', 'Aksi RAB tidak valid.');
        redirect('index.php?page=rab');
    }
}
...
if ($search !== '') {
    $whereSql = " WHERE CONCAT_WS(' ',
        r.kode_rab_1, r.nama_rab_1, r.kode_rab_2, r.nama_rab_2,
        r.kode_rab_3, r.nama_rab_3, r.kode_rab_4, r.nama_rab_4
    ) LIKE :search";
    $queryParams['search'] = '%' . $search . '%';
}

$countStmt = db()->prepare('SELECT COUNT(*) FROM rab r' . $whereSql);
...
$listStmt = db()->prepare(
    'SELECT r.*,
        (SELECT COUNT(*) FROM program p WHERE p.id_rab = r.id) AS program_count,
        (SELECT COUNT(*) FROM dab d WHERE d.id_rab = r.id) AS business_count
     FROM rab r' . $whereSql . '
     ORDER BY r.kode_rab_4 ASC, r.id ASC
...
$stats = db()->query(
    'SELECT COUNT(*) total,
        COUNT(DISTINCT kode_rab_1) level_1,
        COUNT(DISTINCT kode_rab_2) level_2,
        COUNT(DISTINCT kode_rab_3) level_3
     FROM rab'
)->fetch();

function rab_page_url(int $pageNumber, string $search, int $perPage): string
{
...
<div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
    <div>
        ...
        <h1 class="mt-1 text-2xl font-bold tracking-tight text-slate-900">Referensi Arsitektur Bisnis (RAB)</h1>
    ...
    <div class="flex gap-2">
        ...
        <button type="button" class="inline-flex items-center justify-center gap-2 rounded-md bg-blue-700 px-4 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-blue-800" data-rab-add>
            <i data-lucide="plus" class="h-4 w-4"></i>Tambah RAB
        </button>
    </div>
</div>
...
<div class="mb-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
    <?php
    $summaryCards = [
        ['label' => 'Total RAB', 'value' => $stats['total'], 'icon' => 'layers-3'],
...
<section class="rounded-lg border border-slate-200 bg-white shadow-sm">
    <div class="flex flex-col gap-3 border-b border-slate-200 px-4 py-3 lg:flex-row lg:items-center lg:justify-between">
        <div>
            <h2 class="font-semibold text-slate-900">Daftar Referensi Bisnis</h2>
        ...
        <form method="get" class="flex flex-col gap-2 sm:flex-row">
            ...
            <?php if ($search !== ''): ?>
                <a href="index.php?page=rab" class="inline-flex items-center justify-center rounded-md px-3 py-2 text-xs font-semibold text-red-700 hover:bg-red-50">Reset</a>
            <?php endif; ?>
        </form>
    </div>
        ...
        <table class="w-full min-w-[1050px] text-left text-xs xl:min-w-0 xl:table-fixed">
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <?php foreach ($rows as $index => $row): ?>
                    ...
                    $recordPayload = [
                        'id' => (int) $row['id'],
                        'kode_rab_1' => $row['kode_rab_1'],
                        'nama_rab_1' => $row['nama_rab_1'],
                        'kode_rab_2' => $row['kode_rab_2'],
                        'nama_rab_2' => $row['nama_rab_2'],
                        'kode_rab_3' => $row['kode_rab_3'],
                        'nama_rab_3' => $row['nama_rab_3'],
                        'kode_rab_4' => $row['kode_rab_4'],
                        'nama_rab_4' => $row['nama_rab_4'],
                    ...
                    <tr class="align-top hover:bg-slate-50">
                        ...
                        <?php for ($level = 1; $level <= 4; $level++): ?>
                            <td class="break-words px-2.5 py-3">
                                <span class="inline-flex max-w-full break-all rounded bg-slate-100 px-1.5 py-0.5 font-mono text-[10px] font-semibold leading-4 text-slate-700"><?= e($row["kode_rab_{$level}"]) ?></span>
                                <p class="mt-1 text-[11px] leading-4 text-slate-700"><?= e($row["nama_rab_{$level}"]) ?></p>
                            </td>
                        <?php endfor; ?>
                        <td class="px-2 py-3">
                            <div class="flex justify-center gap-1">
                                <button type="button" class="inline-flex h-7 w-7 items-center justify-center rounded border border-slate-300 text-slate-600 hover:bg-slate-100" title="Lihat RAB" aria-label="Lihat <?= e($row['kode_rab_4']) ?>" data-rab-view data-record="<?= e($recordJson) ?>">
                                    <i data-lucide="eye" class="h-3.5 w-3.5"></i>
                                </button>
                                <button type="button" class="inline-flex h-7 w-7 items-center justify-center rounded border border-blue-200 text-blue-700 hover:bg-blue-50" title="Edit RAB" aria-label="Edit <?= e($row['kode_rab_4']) ?>" data-rab-edit data-record="<?= e($recordJson) ?>">
                                    <i data-lucide="pencil" class="h-3.5 w-3.5"></i>
                                </button>
                                <button type="button" class="inline-flex h-7 w-7 items-center justify-center rounded border border-red-200 text-red-700 hover:bg-red-50" title="Hapus RAB" aria-label="Hapus <?= e($row['kode_rab_4']) ?>" data-rab-delete data-id="<?= (int) $row['id'] ?>" data-code="<?= e($row['kode_rab_4']) ?>" data-name="<?= e($row['nama_rab_4']) ?>" data-references="<?= (int) $row['program_count'] + (int) $row['business_count'] ?>">
    ...
    <?php if ($totalRows > 0): ?>
        <div class="flex flex-col gap-3 border-t border-slate-200 px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-5">
            ...
            <?php render_numbered_pagination(
                $currentPage,
                $totalPages,
                static fn(int $pageNumber): string => rab_page_url($pageNumber, $search, $perPage)
            ); ?>
        </div>
    <?php endif; ?>
</section>

<div class="fixed inset-0 z-50 hidden items-center justify-center bg-slate-950/70 px-3 py-4 backdrop-blur-sm sm:px-6" data-modal="rab-form" role="dialog" aria-modal="true" aria-labelledby="rab-form-title">
    <form method="post" class="flex max-h-[94vh] w-full max-w-4xl flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl">
        <?= csrf_field() ?>
        <input type="hidden" name="action" value="<?= e($formMode) ?>" data-rab-action>
        <input type="hidden" name="id" value="<?= e($formState['id']) ?>" data-rab-id>
        ...
        <div class="flex shrink-0 items-start justify-between gap-4 border-b border-slate-200 bg-gradient-to-r from-slate-50 to-white px-5 py-4 sm:px-6 sm:py-5">
            <div class="flex min-w-0 items-start gap-3.5">
                ...
                <div class="min-w-0">
                    <h2 id="rab-form-title" class="text-lg font-bold tracking-tight text-slate-900" data-rab-form-title>Tambah Data RAB</h2>
            ...
            <div class="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
                ...
                <?php for ($level = 1; $level <= 4; $level++): ?>
                    <fieldset class="grid gap-3 border-b border-slate-100 px-4 py-3.5 last:border-b-0 sm:grid-cols-[100px_210px_minmax(0,1fr)] sm:items-center sm:gap-4">
                        ...
                        <div>
                            <label for="kode_rab_<?= $level ?>" class="mb-1.5 block text-[10px] font-semibold uppercase tracking-wide text-slate-500 sm:sr-only">Kode level <?= $level ?></label>
                            <div class="relative">
                                <i data-lucide="hash" class="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400"></i>
                                <input id="kode_rab_<?= $level ?>" name="kode_rab_<?= $level ?>" type="text" maxlength="50" value="<?= e($formState["kode_rab_{$level}"]) ?>" placeholder="RAB.00" class="w-full rounded-lg border border-slate-300 bg-white py-2.5 pl-9 pr-3 font-mono text-sm uppercase text-slate-900 shadow-sm outline-none transition placeholder:text-slate-300 focus:border-blue-600 focus:ring-4 focus:ring-blue-100/70" data-rab-field="kode_rab_<?= $level ?>" required>
                            </div>
                        </div>

                        <div>
                            <label for="nama_rab_<?= $level ?>" class="mb-1.5 block text-[10px] font-semibold uppercase tracking-wide text-slate-500 sm:sr-only">Nama referensi level <?= $level ?></label>
                            <input id="nama_rab_<?= $level ?>" name="nama_rab_<?= $level ?>" type="text" maxlength="255" value="<?= e($formState["nama_rab_{$level}"]) ?>" placeholder="Masukkan nama referensi level <?= $level ?>" class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2.5 text-sm text-slate-900 shadow-sm outline-none transition placeholder:text-slate-300 focus:border-blue-600 focus:ring-4 focus:ring-blue-100/70" data-rab-field="nama_rab_<?= $level ?>" required>
                        </div>
                    </fieldset>
                <?php endfor; ?>
            </div>
        ...
        <div class="flex shrink-0 flex-col-reverse gap-2 border-t border-slate-200 bg-white px-5 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-6">
            ...
            <div class="flex justify-end gap-2.5">
                ...
                <button type="submit" class="inline-flex items-center gap-2 rounded-lg bg-blue-700 px-5 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-blue-800 focus:outline-none focus:ring-4 focus:ring-blue-100">
                    ...
                    <span data-rab-submit-label>Simpan Data</span>
                </button>
            </div>
        </div>
    </form>
...
<div class="fixed inset-0 z-50 hidden items-center justify-center bg-slate-950/70 px-3 py-4 backdrop-blur-sm sm:px-6" data-modal="rab-view" role="dialog" aria-modal="true" aria-labelledby="rab-view-title">
    <div class="flex max-h-[94vh] w-full max-w-4xl flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl">
        <div class="flex shrink-0 items-start justify-between gap-4 border-b border-slate-200 bg-gradient-to-r from-slate-50 to-white px-5 py-4 sm:px-6 sm:py-5">
            <div class="flex min-w-0 items-start gap-3.5">
                ...
                <div class="min-w-0">
                    <h2 id="rab-view-title" class="text-lg font-bold tracking-tight text-slate-900">Detail Data RAB</h2>
            ...
            <?php for ($level = 1; $level <= 4; $level++): ?>
                <section class="relative overflow-hidden rounded-xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
                    ...
                    <div class="relative">
                        ...
                        <p class="mt-4 inline-flex rounded-md bg-red-50 px-2 py-1 font-mono text-xs font-bold text-red-700 ring-1 ring-inset ring-red-100" data-view-field="kode_rab_<?= $level ?>"></p>
                        <p class="mt-2 text-sm font-medium leading-6 text-slate-800" data-view-field="nama_rab_<?= $level ?>"></p>
                    </div>
                </section>
            <?php endfor; ?>
        </div>
        <div class="grid shrink-0 gap-3 border-t border-slate-200 bg-white px-5 py-4 text-sm sm:grid-cols-2 sm:px-6">
            ...
            <div class="flex items-center gap-3 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2.5">
                ...
                <p class="text-slate-600">Proses bisnis terkait <strong class="ml-1 text-slate-900" data-view-businesses>0</strong></p>
            </div>
        </div>
...
<div class="fixed inset-0 z-50 hidden items-center justify-center bg-slate-950/70 px-3 py-4 backdrop-blur-sm sm:px-6" data-modal="rab-delete" role="dialog" aria-modal="true" aria-labelledby="rab-delete-title">
    <form method="post" class="w-full max-w-md overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl">
        ...
        <div class="p-6 sm:p-7">
            ...
            <h2 id="rab-delete-title" class="mt-5 text-xl font-bold tracking-tight text-slate-900">Hapus Data RAB?</h2>
...
(() => {
    ...
    const formModal = document.querySelector('[data-modal="rab-form"]');
    const viewModal = document.querySelector('[data-modal="rab-view"]');
    const deleteModal = document.querySelector('[data-modal="rab-delete"]');
    const fields = <?= json_encode($rabFields, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES) ?>;
    ...
    const fillForm = (record = {}) => {
        fields.forEach((field) => {
            const input = formModal.querySelector(`[data-rab-field="${field}"]`);
            if (input) input.value = record[field] || '';
        });
        formModal.querySelector('[data-rab-id]').value = record.id || '';
    };

    document.querySelector('[data-rab-add]')?.addEventListener('click', () => {
        fillForm();
        formModal.querySelector('[data-rab-action]').value = 'create';
        formModal.querySelector('[data-rab-form-title]').textContent = 'Tambah Data RAB';
        formModal.querySelector('[data-rab-submit-label]').textContent = 'Simpan Data';
        openModal(formModal);
    });

    document.querySelectorAll('[data-rab-edit]').forEach((button) => {
        button.addEventListener('click', () => {
            const record = JSON.parse(button.dataset.record || '{}');
            fillForm(record);
            formModal.querySelector('[data-rab-action]').value = 'update';
            formModal.querySelector('[data-rab-form-title]').textContent = 'Edit Data RAB';
            formModal.querySelector('[data-rab-submit-label]').textContent = 'Simpan Perubahan';
            openModal(formModal);
        });
    });

    document.querySelectorAll('[data-rab-view]').forEach((button) => {
        button.addEventListener('click', () => {
            const record = JSON.parse(button.dataset.record || '{}');
            viewModal.querySelector('[data-view-code]').textContent = `${record.kode_rab_4 || ''} · ${record.nama_rab_4 || ''}`;
            fields.forEach((field) => {
    ...
    document.querySelectorAll('[data-rab-delete]').forEach((button) => {
        button.addEventListener('click', () => {
    ...
    formModal.querySelector('[data-rab-form-title]').textContent = <?= json_encode($formMode === 'update' ? 'Edit Data RAB' : 'Tambah Data RAB') ?>;
    formModal.querySelector('[data-rab-submit-label]').textContent = <?= json_encode($formMode === 'update' ? 'Simpan Perubahan' : 'Simpan Data') ?>;
    openModal(formModal);
    <?php endif; ?>
})();
</script>
